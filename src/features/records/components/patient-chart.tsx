...
import { AnamnesisInviteActions } from "@/features/records/components/anamnesis-invite-actions";
...
import { PostSurgeryMessage } from "@/features/records/components/post-surgery-message";
import type {
  PatientCardSummary,
  PatientChartData,
...
type ChartTab =
  "resumo" | "anamnese" | "odontograma" | "evolucoes" | "pos-cirurgia";
          ...
          <TabsList className="h-auto min-h-0 w-full justify-start gap-0 overflow-x-auto overflow-y-hidden rounded-none rounded-t-xl border-b border-border/60 bg-neo-cream-50/80 p-0 scrollbar-none [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
            ...
            {canViewClinical ? (
              <>
                ...
                <TabsTrigger value="evolucoes" className={chartTabTriggerClass}>
                  Evoluções
                </TabsTrigger>
                <TabsTrigger
                  value="pos-cirurgia"
            ...
            {canViewClinical && chart ? (
              <>
                <TabsContent value="anamnese" className="mt-0 space-y-4">
                  ...
                  {canWriteClinical ? (
                    <>
                      <AnamnesisInviteActions
                        patientId={patient.id}
                        appointmentId={appointmentId}
                        channelConfigured={chart.whatsappChannelConfigured}
                        destination={chart.whatsappDestination}
                      />
                ...
                <TabsContent value="pos-cirurgia" className="mt-0">
                  <PostSurgeryMessage
                    patientId={patient.id}
                    appointmentId={appointmentId}
                    ...
                    channelConfigured={chart.whatsappChannelConfigured}
                    destination={chart.whatsappDestination}
                    messages={chart.postSurgeryMessages}
                  />
                </TabsContent>
              </>
            ) : null}
