...
> Status: **alinhado à spec** `specs/2026-09-23-sugestao-itens-planilha-vision.md` (implementação na branch `feature/sugestao-itens-planilha-vision`)
...
Na **Registrar compra**, a pessoa escolhe a foto da nota/planilha e aperta **Sugerir itens da foto**. O servidor lê a imagem, devolve as linhas e descarta o arquivo. O admin **revisa e confirma**. Só então o sistema cria insumos/pacotes/QR (fluxo atual de entrada).

Valor: menos digitação. A foto **não é histórico**: não entra no Storage, nem em `supply_sheets`, nem em qualquer outra tabela. **Nunca** grava estoque só com IA.

---

## 1. Abordagem (5 passos)

1. **Extrair (servidor, sem persistir a foto)**  
   A action autenticada recebe o arquivo no pedido (JPEG, PNG ou WebP, até 10 MB). Envia os bytes a um modelo **vision** da OpenAI (ex.: `gpt-4o`), com prompt estrito pedindo JSON tipado: nome, quantidade por pacote, n° de pacotes, lote, validade, unidade sugerida. Não faz upload em `supply-sheets`. Não insere em `supply_sheets`. Não grava a imagem em disco, banco ou log. Sem gravação de estoque nesta etapa. Ao terminar a chamada, os bytes saem de memória.
...
- Persistir a foto da sugestão (bucket, tabela, disco ou log)
...
| Custo e latência OpenAI                | Só sob demanda (botão); timeout; teto de 10 MB no pedido, igual ao upload atual |
...
| Upload `supply-sheets`                    | **Não**            | A sugestão não grava a foto. O passo 1 atual do wizard (foto histórica) continua gravando se a pessoa usar esse caminho |
...
4. **Fechado (23/09/2026):** a foto da sugestão não é armazenada em nenhum lugar nosso. Só trafega no pedido até a OpenAI e é descartada.
